...
class Aligner:
    def __init__(self, db, soap_db):
        self._db = db
        self._soap_db = soap_db
    ...
    def align(self, sequence_file, algorithm):
        ...
        elif algorithm is Algorithm.SOAP2:
            return self._soap(sequence_file)
    ...
    def _soap(self, sequence_file):
        new_filename = 'sequence_soap.fasta'
        ...
        with open(sequence_file) as f:
            ...
            for i in range(1, len(lines)):
                new_lines[-1] += "".join(lines[i].split())
        ...
        with open(self._soap_db) as db:
            for db_item in db.readlines():
                if os.path.exists(output_file):
                    os.remove(output_file)

                soap_cmd = ['soap', '-a', new_filename, '-D', db_item.strip(), '-o', output_file, '-M', '0']
                subprocess.run(soap_cmd)
                ...
                with open(output_file) as f:
                    ...
                    if len(lines):
                        ...
                        html += self._get_html(db_item[2:-7], result[5], result[1], match, subject)
